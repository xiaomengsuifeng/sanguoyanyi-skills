...
OUTPUT_FILE = os.path.join(SCRIPT_DIR, "司马懿全文提取.txt")

def extract_simayi_content():
    keywords = ["司马懿", "仲达", "司马仲达", "宣王"]

    files = sorted([f for f in os.listdir(YUANZHU_DIR) if f.endswith('.txt')])

    all_content = []
    all_content.append("=" * 80)
    all_content.append("三国演义 - 司马懿相关段落全文提取")
    all_content.append("=" * 80)
    all_content.append("")
    ...
    print(f"司马懿出现的回数: {count}/120")

if __name__ == "__main__":
    extract_simayi_content()