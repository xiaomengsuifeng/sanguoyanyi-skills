...
OUTPUT_FILE = os.path.join(SCRIPT_DIR, "周瑜全文提取.txt")

def extract_zhouyu_content():
    keywords = ["周瑜", "公瑾", "周公瑾", "周都督"]

    files = sorted([f for f in os.listdir(YUANZHU_DIR) if f.endswith('.txt')])

    all_content = []
    all_content.append("=" * 80)
    all_content.append("三国演义 - 周瑜相关段落全文提取")
    all_content.append("=" * 80)
    all_content.append("")
    ...
    print(f"周瑜出现的回数: {count}/120")

if __name__ == "__main__":
    extract_zhouyu_content()