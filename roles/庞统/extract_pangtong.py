...
OUTPUT_FILE = os.path.join(SCRIPT_DIR, "庞统全文提取.txt")

def extract_pangtong_content():
    keywords = ["庞统", "士元", "庞士元", "凤雏"]

    files = sorted([f for f in os.listdir(YUANZHU_DIR) if f.endswith('.txt')])

    all_content = []
    all_content.append("=" * 80)
    all_content.append("三国演义 - 庞统相关段落全文提取")
    all_content.append("=" * 80)
    all_content.append("")
    ...
    print(f"庞统出现的回数: {count}/120")

if __name__ == "__main__":
    extract_pangtong_content()